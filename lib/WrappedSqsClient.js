...
export class WrappedSqsClient {
  ...
  constructor({
    ...
    this.#client = process.env.AWS_XRAY_DAEMON_ADDRESS
      ? AWSXray.captureAWSv3Client(sqs)
      : sqs;
  }
  ...
  static decodeMessage(message = {}) {
    ...
    eventSourceArn ??= EventSourceARN ?? eventSourceARN;
    ...
    return {
      ...
      ...(_.isNil(awsRegion) ? {} : { awsRegion }),
      ...(_.isNil(body) ? {} : { body }),
      ...(_.isNil(eventSource) ? {} : { eventSource }),
      ...(_.isNil(eventSourceArn) ? {} : { eventSourceArn }),
      ...(_.isEmpty(messageAttributes)
        ? {}
        : {
            messageAttributes:
              WrappedSqsClient.decodeMessageAttributes(messageAttributes),
          }),
      ...(_.isNil(messageId) ? {} : { messageId }),
      ...(_.isNil(receiptHandle) ? {} : { receiptHandle }),
    };
  }
   ...
   *
   * @param {string[]} [attributeNames] - {@link https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/client/sqs/command/GetQueueAttributesCommand/ GetQueueAttributesCommand}
   * @return {Promise<object>} Queue attributes.
   */

  async getQueueAttributes(attributeNames = ['All']) {
    // Validate state.
    if (!this.queueInitialized) throw new Error('queue not yet initialized');

    // Send command.
    this.#logger.debug(
      `Retrieving attributes for queue '${this.#queueName}'...`
    );
    ...
    this.#logger.debug(`Retrieved attributes.`, response);
   ...
   * @param {DecodedMessageAttributes} [options.messageAttributes] - {@link DecodedMessageAttributes Message attributes.}
  ...
  async sendMessage({
    ...
    const message = {
      ...
      MessageAttributes:
        this.constructor.encodeMessageAttributes(messageAttributes),
   ...
   * @property {string} [deduplicationId] - {@link https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/clients/client-sqs/interfaces/sendmessagecommandinput.html#messagededuplicationid SendMessageCommandInput messageDeduplicationId}
   * @property {number} [delaySeconds] - Delay in seconds.
   * @property {string} [groupId] - {@link https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/clients/client-sqs/interfaces/sendmessagecommandinput.html#messagegroupid SendMessageCommandInput messageGroupId}
   * @property {string} id - Message ID.
   * @property {DecodedMessageAttributes} [messageAttributes] - {@link DecodedMessageAttributes Message attributes.}
   * @property {DecodedMessageAttributes} [systemAttributes] - {@link https://docs.aws.amazon.com/AWSJavaScriptSDK/v3/latest/clients/client-sqs/interfaces/sendmessagecommandinput.html#messagesystemattributes SendMessageCommandInput messageSystemAttributes}
   */
